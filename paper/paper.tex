...
the one-scene passages, the two ablations agree, which is consistent with that explanation. Our
contributions in this paper can be summarized as follows:

\begin{itemize}
\item A statement of what an image-ablated control must do to attribute a score change to the image: keep the model responding about the scored objects, and change image content rather than remove the image (Section~\ref{sec:theory}).

\item Evidence on two standard benchmarks that the blank fails that test while a mismatched image passes it, with a published method's gain decomposed into its rates (Section~\ref{sec:mitigation}).

\item A controlled case in which the choice of ablation changes the answer, with the reason and the hit and false-alarm rates that show how the image's contributions cancel inside the score (Section~\ref{sec:results}), and a reporting
...
Under an equal-variance Gaussian formulation the
...
Let $\Delta(x)$ be the change in $J$ that an
...
Under a grey field, and equally under a black one, the model names
...
At PAI's prescribed strength, which we use throughout (at the
...
What the decomposition adds is that the rate of naming absent objects also halves (Figure~\ref{fig:hf}), so CHAIR improves
...
On the registered sample the mismatched
...
The disagreement between ablations is shown on \textsc{LLaVA-1.5-7B}. On
...
$\dSight$ is the difference between the image's
...
ablations above the substitution leave the one-scene $J$ within $0.06$ of its sighted $+0.188$, and the
four below it collapse it to near zero. Only the substitution moves $\dSight$ off zero. Intervals come from this experiment's own resample, so the grey row differs in
...
\toprule
Ablation & Severity & $J$, one scene & $J$, five scenes & $\dBlind$ & $\dSight$ \\
\midrule
...
Of the ten ablations run on the registered sample
(Table~\ref{tab:ladder}; Appendix~\ref{app:ladder}), five barely remove the image: under pixel noise, light blur and patch
...
\label{sec:decomp}Under either ablation the image changes the rates, but differently
...
The check that removes the image to see whether a grounding gain survives is only as good as what
replaces the image. On CHAIR and POPE a blank cannot run it at all: the model names nothing, or answers
``no'' to every question, so the score returns no data rather than a null. A mismatched real image can
run it, and under it a published method's gain is tied to the image while decomposing mainly into a
threshold shift. In a constructed case whose cause is textual, the two ablations disagree: the grey
field attributes none of a large rise to the image and the mismatched image about a third. The
disagreement follows from where the model sits without the image, because a blank removes the
answerable content along with the evidence; on a model that is already near chance on the one-scene
passages, the two agree.

Three practices follow. Report the hit and false-alarm rates with their image-attributable parts rather
than a single score, because the image's effects on the two can cancel inside it. Ablate with an image
whose content differs rather than with a blank, and report the ablated arm's levels, so that a reader
can see whether the control blinded the model or silenced it. And state which ablation and which
generation budget produced an attribution, since either choice can change the answer.
...
AI tools were used as supplementary aids during this work, primarily for developing an outline and structural scaffolding for the manuscript and for assisting with programming and debugging. The authors independently determined the research direction, methodology, experimental setup, analysis, and conclusions. Any AI assisted code or writing was critically reviewed, adapted as needed, and verified by the authors, who take full responsibility for the final manuscript and reported results.
