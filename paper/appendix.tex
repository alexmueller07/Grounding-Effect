...
present and absent units and $J = H - F$. Two older terms remain here: \emph{blind} denotes the
grey-field arm, the originally registered ablation, and a \emph{lane} is one registered experiment
with its own code and gates. Registered outcome labels are set in small capitals
...
conditions: dropping the budgettion-mined half moves $91$ of $8{,}740$ units ($1.04\%$) on the first
architecture and $109$ of $10{,}334$ ($1.05\%$) on the second, affecting $6.4\%$ of union-true units
in \emph{both} conditions on \textsc{LLaVA-1.5-7B} and $7.4\%$ and $6.7\%$ on the second. A
...
\textsc{LLaVA-OV-0.5B} and fails on five of sixteen powered blocks on the first --- four
...
estimate of $\dBlind / \dPrim$ is $0.7333$ on \textsc{LLaVA-1.5-7B} and $0.9074$ on the second. These
...
budget binds.} With shares at the budget of $0.447$ and $0.679$ in the two conditions of the first
architecture, a large fraction of continuations end at the budget, so a unit's carry probability depends
...
\begin{table}[htbp]
\caption{First commit of each registration (short hash, date, Central European Summer Time). Row
numbers follow Table~\ref{tab:registry}. Later amendments are separate, dated commits.}
\label{tab:reghash}
\begin{center}
\small
\begin{tabular}{rlll}
...
13 & De-saturated forced choice                   & not committed; cluster copy & 2026-09-15 \\
...
17 & Ablation ladder (mismatched image: Amendment~2, \texttt{463fe01}) & \texttt{f186bcf} & 2026-09-19 \\
...
\raggedright registered cell fired on both; $\dSight$ null on both; zero-$J$ flag \textsc{fails} &
...
sample, and the mismatched-image arm on \textsc{Qwen3-VL-8B} were added after the corresponding lanes
...
yes-rate $0.5581$ --- and also why the registered instrument gate fired on it: its no-prefix $J$ was
...
attributable to the image. The two $J$ columns show whether an ablation blinded the model: the five
rungs above the substitution leave the one-scene $J$ within $0.06$ of its sighted $+0.188$, and the
four below it collapse it to near zero. Only the substitution moves $\dSight$ off zero. Intervals come from this lane's own resample, so the grey row differs in
...
\toprule
Ablation & Severity & $J$, one scene & $J$, five scenes & $\Delta_{\mathrm{rung}}$ & $\dSight$ \\
\midrule
\texttt{identity}   & $0.000$ & $+0.188$ & $+0.674$ & $+0.486$ & (reference) \\
\texttt{noise16}    & $0.008$ & $+0.177$ & $+0.670$ & $+0.493$ & $-0.008$ \civ{-0.038}{+0.022} \\
\texttt{noise32}    & $0.016$ & $+0.176$ & $+0.669$ & $+0.492$ & $-0.007$ \civ{-0.045}{+0.030} \\
\texttt{noise64}    & $0.034$ & $+0.164$ & $+0.648$ & $+0.485$ & $+0.001$ \civ{-0.036}{+0.039} \\
\texttt{blur4}      & $0.044$ & $+0.154$ & $+0.652$ & $+0.498$ & $-0.013$ \civ{-0.054}{+0.031} \\
\texttt{shuffle24}  & $0.079$ & $+0.137$ & $+0.635$ & $+0.498$ & $-0.012$ \civ{-0.063}{+0.041} \\
\texttt{mismatch}   & $0.080$ & $+0.124$ & $+0.450$ & $+0.326$ & $\mathbf{+0.160}$ \civ{+0.105}{+0.213} \\
\texttt{lowres16}   & $0.089$ & $+0.025$ & $+0.514$ & $+0.489$ & $-0.004$ \civ{-0.055}{+0.048} \\
\texttt{black}      & $0.094$ & $+0.007$ & $+0.485$ & $+0.477$ & $+0.009$ \civ{-0.049}{+0.067} \\
\texttt{grey}       & $0.102$ & $-0.022$ & $+0.480$ & $+0.502$ & $-0.016$ \civ{-0.073}{+0.041} \\
\texttt{blur16}     & $0.126$ & $+0.020$ & $+0.505$ & $+0.485$ & $+0.001$ \civ{-0.055}{+0.055} \\
\bottomrule
\end{tabular}
\end{center}
\end{table}
...
of $+5.56$ in $\mathrm{CHAIR}_i$ (this lane's resample gives \civ{+3.88}{+7.18}, the PAI lane's
...
\paragraph{A mismatched-image arm (post hoc).} After the registered lane, we ran the substitution of
...
\midrule
Untruncated & Sighted          & $+0.105$ & $+0.451$ & $+0.346$ & (reference) & --- \\
