...
The lane above registered one method. A second, \textsc{vcd} \citep{vcd}, was dropped from that lane
by Amendment~1 on four facts verified at primary source and then run under a separate registration
with its own positive control; a reader meeting only the amendment would otherwise conclude it was
never run. It is reported here because it is a registered null, not because it carries any claim.
...
lane's greedy \texttt{vanilla} and the third against a sampled \texttt{vanilla\_s}; no contrast
...
\section{The blind arm in the uncapped regime}
...
$+0.486$ to $+0.346$. That control had no blind arm, so the blind comparison in the main text was
originally available only in the truncated configuration. This appendix supplies the missing arm.

\paragraph{Construction.} The blind generator is the sighted generator with one line changed: the
target image is replaced by a flat mid-grey field of its own pixel dimensions, through the same
mirrored code path used for the published blind arms. Prefix token ids, cell construction, sort
order and batch size are untouched. Three gates ran before any blind row was kept, on all four
blocks: the mirror gate, which requires the blind code path with the grey substitution \emph{off}
to reproduce the sighted generator token-for-token (\textbf{32/32} identical); a prefix-drift gate
against the published cells (\textbf{0} of $720$ published prefixes drift); and a census gate on
the cell md5s (\textbf{0} of $5{,}250$ per block). $21{,}000$ blind rows over the same $10{,}500$
cells as the sighted run.

\paragraph{Cell set and positive control.} The scored population is fixed by the \emph{registered}
regime rule applied to the sighted arms only --- both sighted arms uncapped --- so that the sighted
and blind contrasts are computed over an identical set of $2{,}089$ cells on $1{,}649$ images. On
that set the sighted contrast is $+0.3460$, reproducing the registered value in
Table~\ref{tab:regime} to four decimals, which is the check that the cell selection and the scorer
match the published ones.

\paragraph{Result.} Gaps are $+0.105$ (one scene) and $+0.451$ (five scenes) sighted, against
$-0.069$ and $+0.177$ blind. The contrasts are $+0.346$ and $+0.246$, so

\begin{center}
$\dSight = +0.100$ \civ{+0.041}{+0.163}, \qquad blind/sighted $= 0.710$,
\end{center}

with $B = 4000$ paired bootstrap replicates clustered on the image. The interval excludes zero,
where the same grey ablation in the truncated regime gives $-0.016$ \civ{-0.073}{+0.041} at a ratio
of $1.03$.

\paragraph{Reading.} The published null is therefore specific to the regime in which it was
measured: capping compresses the blind arm's one-scene hit rate and preserves the difference while
destroying the level, exactly as the substitution analysis of Appendix~\ref{app:ladder} finds for
the blank itself. Two corrections that are independent of one another --- changing the ablation, or
removing the truncation --- move the ratio from $1.03$ to $0.67$ and $0.71$ respectively. We read
the agreement as the substantive finding and neither number as a precise estimate of the image's
share.

\paragraph{What this does not settle.} The blind arm was added after the regime control was
registered and analysed, so it is a post-hoc addition to that lane and is labelled as such. It uses
the grey ablation, which Appendix~\ref{app:ladder} shows is not the most informative choice; a
substitution arm in the uncapped regime was not run. And the cells it scores are those that
terminate naturally under \emph{both} sighted conditions, a population that is easier to caption
