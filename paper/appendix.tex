...
architecture and $109$ of $10{,}334$ ($1.05\%$) on the second, affecting $6.4\%$ of union-true units
...
$-12.68$ \civ{-14.77}{-10.56}, on the first architecture).
...
distinction concrete: on the first architecture they average $26.59$ object mentions across only
...
threshold cell; the fourth step's point estimate slightly exceeds the third's, so the curve is
...
readout mechanically \textsc{replicates}, licence text set aside; $\dSight$ excludes zero downward &
...
\raggedright seven graded rungs \textsc{ladder-null}; substitution \textsc{image-contributes} (after Amendment~3); on CHAIR, substitution \textsc{gain-is-image-attributable} &
\raggedright Sec.~\ref{sec:blind}, \ref{sec:mitigation}; App.~\ref{app:ladder} \tabularnewline\addlinespace[1pt]
...
\raggedright attention-only arm \textsc{partial}; released default no gain; grey \textsc{blind-arm-degenerate} &
...
\raggedright gate \textsc{run-endpoint}; registered cell \textsc{image-attributable} &
...
proved specific to assembled captions. The title under which these lanes were registered was
...
arm's, from \textsc{confounded-by-collapse} to \textsc{image-contributes} (Appendix~\ref{app:ladder}).
\item Both uncapped-regime ablation arms, and the extension of both ablations to the $3{,}500$-image
sample, were added after the regime control was registered and analysed; they are post hoc
(Section~\ref{sec:blind}; Appendix~\ref{app:regimeblind}).
...
the paper be retitled; it was, and the present title makes no claim about the construction. Both natural-prose conditions sit at gap $+0.4221$ and $+0.4283$,
...
generations requires a single consumer \textsc{gpu} and the five public checkpoints of
...
\begin{figure}[t]
{\centering
\includegraphics{fig_hf.pdf}\par}
...
three arms ($10{,}500$ cells, $3{,}500$ images). ``Uncapped'' applies the \emph{registered} regime rule
...
runs to four decimals. The rule does not constrain the ablated arms: inside the uncapped cells grey
...
\begin{table}[h]
\caption{The one-to-five-scene contrast under each ablation on the $3{,}500$-image sample. Gaps are
$J = H - F$ within each condition. Intervals are $B = 4000$ bootstrap replicates clustered on the
image; ratio intervals are percentile intervals of the replicate ratios.}
...
\toprule
Cells & Arm & One scene & Five scenes & Contrast & $\dSight$ & Ablated/sighted \\
\midrule
All      & Sighted          & $+0.176$ & $+0.647$ & $+0.472$ & (reference) & --- \\
($10{,}500$) & Grey field   & $-0.028$ & $+0.447$ & $+0.475$ & $-0.003$ \civ{-0.025}{+0.019} & $1.01$ \civ{0.96}{1.05} \\
         & Mismatched image & $+0.099$ & $+0.418$ & $+0.319$ & $+0.153$ \civ{+0.132}{+0.175} & $0.68$ \civ{0.64}{0.72} \\
\midrule
Uncapped & Sighted          & $+0.105$ & $+0.451$ & $+0.346$ & (reference) & --- \\
($2{,}089$) & Grey field    & $-0.069$ & $+0.177$ & $+0.246$ & $+0.100$ \civ{+0.041}{+0.163} & $0.71$ \civ{0.56}{0.87} \\
         & Mismatched image & $+0.057$ & $+0.221$ & $+0.164$ & $\mathbf{+0.182}$ \civ{+0.122}{+0.245} & $0.47$ \civ{0.33}{0.62} \\
\bottomrule
\end{tabular}
\end{center}
\end{table}
...
images played no part in either. Restricting to uncapped cells moves both
(Table~\ref{tab:regimeblind}). Under grey the uncapped one-scene level falls below zero ($-0.069$
...
($41$--$42\%$ uncapped) name an object that the substitute image also contains, against $10$--$12\%$
...
\civ{0.40}{0.52}) and to $+0.266$ \civ{+0.192}{+0.342} on uncapped cells (ratio $0.22$
...
\paragraph{What this does not settle.} The substitution uses one derangement. The uncapped cells are
