...
Four checkpoints are used; the first two carry every primary number. The first architecture is
...
The two further models were each run under their own registration. \textsc{Qwen2.5-VL-7B} is
...
\begin{table}[htb]
\caption{The contrast under three independent ground-truth constructions. The six $\dSight$ point
estimates lie between $-0.042$ and $-0.016$, and in all six $\dBlind$ exceeds $\dPrim$; intervals are
...
condition reached the generation cap, at $3{,}500$ images.}
\label{tab:regime}
\begin{center}
\footnotesize
...
the same, $192$-token reconstruction & $+0.3460$ & \civ{+0.2849}{+0.4058} & $0.1209$ \\
...
\begin{table}[htb]
\caption{The rise with and without the image, for all four architectures run. The blind rise is
...
the image-attributable component of $d'$ for all four architectures is given in the text below.}
\label{tab:dprime}
\begin{center}
\small
\begin{tabular}{lllll}
...
The blind arm was registered and run, and it does not yield a contrast.
...
\caption{Experiment~3's scene-count contrast in two registers, with $95\%$ intervals.}
\label{fig:forced}
\end{minipage}\hfill
...
\paragraph{The objection.} Experiment~3 (Section~\ref{sec:forced}) moves the scene-count contrast by
