...
def main():
    ...
    blind_paths = [f"{C.OUT_R}/at_corrobpw_b{b}_blind.jsonl" for b in range(C.N_BLOCKS)]
    have_blind = all(os.path.isfile(p) for p in blind_paths)
    B = load(blind_paths) if have_blind else []
    print(f"[cpwbs] sighted rows {len(S)}  blind rows {len(B)}  blind_present={have_blind}", flush=True)
    ...
    vocab = {}
    print(f"[cpwbs] cells both-sighted-uncapped: {len(keys)}  images {len(imgs)}", flush=True)
    ...
    print(f"[cpwbs] SIGHTED  gap(cap1)={s1:+.4f} gap(cap5)={s5:+.4f}  Delta={dS:+.4f}", flush=True)
    print(f"[cpwbs] POSITIVE CONTROL vs published +0.3460 -> dev {abs(dS-0.3460):.4f}", flush=True)
    ...
    if have_blind:
        ...
        print(f"[cpwbs] BLIND    gap(cap1)={b1:+.4f} gap(cap5)={b5:+.4f}  Delta={dB:+.4f}", flush=True)
        print(f"[cpwbs] DELTA_IMAGE {dS-dB:+.4f}   ratio {dB/dS:.3f}", flush=True)
        ...
        print(f"[cpwbs] DELTA_IMAGE 95% CI [{lo:+.4f}, {hi:+.4f}]  (B={len(ds)}, clustered on image)", flush=True)

    json.dump(out, open(f"{C.OUT_R}/cpw_blind_score.json", "w"), indent=1)
    print("CPWBS_SENTINEL_OK", flush=True)
