...
from wia_blind_common import gen_batch_gray, mirror_gate_f1
...
def main():
    ...
    print(f"[cpwgB] MAX_NEW_CONT override {MAX_NEW_REF} -> {wia_lngen.MAX_NEW_CONT}", flush=True)
    ...
    print(f"[cpwgB] BLOCK={BLOCK} smoke={SMOKE} cells {len(cells)} of {len(cells_all)}", flush=True)
    ...
    print(f"[cpwgB] MODEL={MODEL} tokenizer={tok.__class__.__name__} vocab={tok.vocab_size} "
    ...
    print(f"[cpwgB] CONSTRUCTION GATE: rebuilt {nchk}, mismatches {nbad}", flush=True)
    ...
    print(f"[cpwgB] SRCS GATE drift {st['drift_srcs']}", flush=True)
    print(f"[cpwgB] PREFIX GATE drift {st['drift_prefix']} / {2*st['npub']} published prefixes",
          flush=True)
    print(f"[cpwgB] CENSUS MD5 GATE drift {st['drift_md5']} / {2*st['ncell']}", flush=True)
    ...
    print(f"[cpwgB] cells {st['ncell']} ({st['npub']} published) rows {len(items)} "
    ...
    bs = max(4, BS // 2)
    nchk, nid = mirror_gate_f1(model, proc, tok, items[:32], bs, MAX_NEW_384)
    print(f"[cpwgB] GATE_MIRROR_IDENTICAL {nid}/{nchk}", flush=True)
    assert nchk > 0 and nid == nchk, f"FATAL_MIRROR_NOT_IDENTICAL {nid}/{nchk}"
    t0 = time.time()
    for b0 in range(0, len(items), bs):
        chunk = items[b0:b0 + bs]
        conts = gen_batch_gray(model, proc, tok, chunk, gray=True, max_new=MAX_NEW_384)
        ...
        if (b0 // bs) % 50 == 0 or b0 + len(chunk) >= len(items):
            ...
            print(f"[cpwgB] b{BLOCK} {done}/{len(items)} t={el:.0f}s "
    ...
    print(f"[cpwgB] 192-RECONSTRUCTION AGREEMENT vs at_redun.jsonl on published cells "
    ...
    print(f"[cpwgB] mean cont len {cl}", flush=True)
    print(f"[cpwgB] at_max(384) {am}   empty {emp}", flush=True)
