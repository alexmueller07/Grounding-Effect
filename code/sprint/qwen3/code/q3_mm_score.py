...
def main():
    ...
    if not all(v["pass"] for v in res["positive_control"].values()):
        ...
        json.dump(res, open(f"{OUT}/q3_mm_score.json", "w"), indent=1)
        print("[score] CELL = NO-DATA (positive control failed)", flush=True)
        sentinel("SCORE")
        return
    if os.environ.get("Q3_CTL_ONLY") == "1":
        json.dump(res, open(f"{OUT}/q3_mm_ctl_only.json", "w"), indent=1)
        sentinel("CTL_ONLY")
        return
    ...
    rows = jload(f"{OUT}/q3_sighted.jsonl") + jload(f"{OUT}/q3_mm.jsonl")
    by_cell = {}
    for r in rows:
    ...
    g["blind_prefix_identical"] = {
        "cap1": sum(by_cell[k]["cap1m"]["prefix_text"] == by_cell[k]["cap1"]["prefix_text"]
                    for k in keys),
        "cap5": sum(by_cell[k]["cap5m"]["prefix_text"] == by_cell[k]["cap5"]["prefix_text"]
                    for k in keys), "n": len(keys)}
    g["zero_doubling_five_scene"] = all(by_cell[k][a]["cycles"] == 0
                                        for k in keys for a in ("cap5", "cap5m"))
    g["one_scene_doubling_rate"] = float(np.mean([by_cell[k]["cap1"]["cycles"] > 0
                                                  for k in keys]))
    g["unit_count_cap1_sources"] = all(by_cell[k][a]["n_src"] == 1
                                       for k in keys for a in ("cap1", "cap1m"))
    ...
    # mismatch arms are NOT grey: every arm must carry gray=False, and every mismatch row must
    # have been generated from a different sample image than its target
    g["gray_flag_correct"] = all(by_cell[k][a]["gray"] is False for k in keys for a in ALL)
    g["mm_substituted"] = all(by_cell[k][a].get("sub_image_id") not in (None, k[0])
                              for k in keys for a in BLIND)
    hard = ["four_arms_every_cell", "budget_equal_within_cell", "plen_equals_budget",
            ...
            "cont_len_consistent", "gray_flag_correct", "mm_substituted"]
    ...
    C["BLIND"] = contrast("BLIND", UB["cap1m"], UB["cap5m"], n_img, extra={"n_cells": len(ksB)})
    UBK, _, _ = bt(BLIND, klen=True)
    C["BLIND_klen"] = contrast("BLIND_klen", UBK["cap1m"], UBK["cap5m"], n_img)
    ...
    def joint(sel):
        ...
        c = gap_raw(UB["cap1m"], UB["cap1m"].gather(sel))[0]
        d = gap_raw(UB["cap5m"], UB["cap5m"].gather(sel))[0]
        if None in (a, b, c, d):
            return None
        return (b - a) - (d - c)
    ...
    for cond, (sa, ba) in (("one_scene", ("cap1", "cap1m")), ("five_scene", ("cap5", "cap5m"))):
